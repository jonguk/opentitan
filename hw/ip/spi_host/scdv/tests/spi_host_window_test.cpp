...
#include "spi_host_window_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class spi_host_window_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(spi_host_window_test);
  spi_host_env* m_env {};
  explicit spi_host_window_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = spi_host_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = spi_host_window_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(spi_host_window_test);
