...
class spi_host_csr_rw_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(spi_host_csr_rw_seq);
  explicit spi_host_csr_rw_seq(const std::string &nm = "spi_host_csr_rw_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    const uint32_t offs[] = { 0, 4, 8, 12, 0, 4, 8, 12, 16, 28, 32, 36 };
    const uint32_t wmask[] = { 0x0, 0xffffffff, 0x0, 0x0, 0xe000ffff, 0x0, 0xefffffff, 0xffffffff, 0x0, 0x1f, 0x0, 0x3f };
    const uint32_t womask[] = { 0x0, 0x0, 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1ffffff, 0x0, 0x0, 0x0 };
    const uint32_t w1cmask[] = { 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3f, 0x0 };
    const uint32_t rcmask[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
