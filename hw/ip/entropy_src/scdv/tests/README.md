...
2) Build all tests:
   ...
   make -C hw/ip/entropy_src/scdv/tests all

3) Run a specific test (UVM style):

   cd hw/ip/entropy_src/scdv/tests
   ./$entropy_src_csr_rw_test +UVM_TESTNAME=$entropy_src_csr_rw_test +UVM_VERBOSITY=UVM_MEDIUM +ntb_random_seed=random
...
4) Generate coverage HTML (parallel, accumulate across repeats/seeds):
   ...
   util/scdv_cov_report.py --ip entropy_src --outdir hw/ip/entropy_src/scdv/tests/coverage_html      --systemc-home "$SYSTEMC_HOME" --jobs 4 --repeat 1 --extra-args "+UVM_VERBOSITY=UVM_LOW +ntb_random_seed=random" --accumulate --rich
...
Template verification:
 - To verify generated TLM matches checked-in sources, use:
   ...
   util/sc_tlm_diff.py --ip entropy_src  # add --update to sync

Collateral generation:
 - Dry-run scaffold & diff:

   util/gen_scdv_collateral.py --tmp-out --skip-regtool entropy_src

 - Apply scaffold (writes to repo):

   util/gen_scdv_collateral.py entropy_src
