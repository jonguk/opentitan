...
class entropy_src_racl_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(entropy_src_racl_seq);
  explicit entropy_src_racl_seq(const std::string &nm = "entropy_src_racl_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    if (tl_bind::set_reg_policy) tl_bind::set_reg_policy(3, true, false);
