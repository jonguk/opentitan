...
class soc_dbg_ctrl_csr_rw_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(soc_dbg_ctrl_csr_rw_seq);
  explicit soc_dbg_ctrl_csr_rw_seq(const std::string &nm = "soc_dbg_ctrl_csr_rw_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    const uint32_t offs[] = { 12 };
    const uint32_t wmask[] = { 0x0 };
    const uint32_t womask[] = { 0x0 };
    const uint32_t w1cmask[] = { 0x0 };
    const uint32_t rcmask[] = { 0x0 };
