...
class dma_racl_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(dma_racl_seq);
  explicit dma_racl_seq(const std::string &nm = "dma_racl_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    wr_blk->a_opcode = tl_a_op_e::PutFullData; wr_blk->a_addr = 20; wr_blk->a_size = 2; wr_blk->a_mask = 0xF; wr_blk->a_data = 0xA5A5u;
    start_item(wr_blk); finish_item(wr_blk);
    tl_item *rd_chk = tl_item::type_id::create("rd_chk", nullptr);
    rd_chk->a_opcode = tl_a_op_e::Get; rd_chk->a_addr = 20; rd_chk->a_size = 2; rd_chk->a_mask = 0xF;
    ...
    wr_ok->a_opcode = tl_a_op_e::PutFullData; wr_ok->a_addr = 20; wr_ok->a_size = 2; wr_ok->a_mask = 0xF; wr_ok->a_data = 0x5A5Au;
    start_item(wr_ok); finish_item(wr_ok);
    tl_item *rd_ok = tl_item::type_id::create("rd_ok", nullptr);
    rd_ok->a_opcode = tl_a_op_e::Get; rd_ok->a_addr = 20; rd_ok->a_size = 2; rd_ok->a_mask = 0xF;
