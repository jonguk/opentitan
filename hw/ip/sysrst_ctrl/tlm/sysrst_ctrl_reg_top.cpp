...
#include "sysrst_ctrl_reg_pkg.hpp"
#include <cstring>

namespace opentitan {
namespace sysrst_ctrl {

sysrst_ctrl_reg_top::sysrst_ctrl_reg_top(sc_core::sc_module_name name)
    : sc_core::sc_module(name), tlm_target("tlm_target") {
  tlm_target.register_b_transport(this, &sysrst_ctrl_reg_top::b_transport);
  reset();
}

void sysrst_ctrl_reg_top::reset() {
...
void sysrst_ctrl_reg_top::sw_predict_write(uint32_t addr, uint32_t data) {
  (void)apply_sw_write(addr, data);
}

uint32_t sysrst_ctrl_reg_top::sw_read(uint32_t addr) {
  return read_sw(addr);
}

void sysrst_ctrl_reg_top::b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
  (void)delay;
...
bool sysrst_ctrl_reg_top::check_racl(bool is_write, uint32_t addr) const {
  if (addr < 172) {
...
bool sysrst_ctrl_reg_top::apply_sw_write(uint32_t addr, uint32_t wdata) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 172) {
...
uint32_t sysrst_ctrl_reg_top::read_sw(uint32_t addr) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 172) {
    auto &regs = regs_default_;
...
void sysrst_ctrl_reg_top::set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write) {
