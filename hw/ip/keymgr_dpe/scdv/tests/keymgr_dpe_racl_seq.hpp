...
class keymgr_dpe_racl_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(keymgr_dpe_racl_seq);
  explicit keymgr_dpe_racl_seq(const std::string &nm = "keymgr_dpe_racl_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    if (tl_bind::set_reg_policy) tl_bind::set_reg_policy(0, true, false);
    ...
    wr_ok->a_opcode = tl_a_op_e::PutFullData; wr_ok->a_addr = 0; wr_ok->a_size = 2; wr_ok->a_mask = 0xF; wr_ok->a_data = 0x5A5Au;
    start_item(wr_ok); finish_item(wr_ok);
    tl_item *rd_ok = tl_item::type_id::create("rd_ok", nullptr);
    rd_ok->a_opcode = tl_a_op_e::Get; rd_ok->a_addr = 0; rd_ok->a_size = 2; rd_ok->a_mask = 0xF;
