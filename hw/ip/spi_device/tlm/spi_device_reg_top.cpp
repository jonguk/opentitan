...
#include "spi_device_reg_pkg.hpp"
#include <cstring>

namespace opentitan {
namespace spi_device {

spi_device_reg_top::spi_device_reg_top(sc_core::sc_module_name name)
    : sc_core::sc_module(name), tlm_target("tlm_target") {
  tlm_target.register_b_transport(this, &spi_device_reg_top::b_transport);
  reset();
}

void spi_device_reg_top::reset() {
...
void spi_device_reg_top::sw_predict_write(uint32_t addr, uint32_t data) {
  (void)apply_sw_write(addr, data);
}

uint32_t spi_device_reg_top::sw_read(uint32_t addr) {
  return read_sw(addr);
}

void spi_device_reg_top::b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
  (void)delay;
...
bool spi_device_reg_top::check_racl(bool is_write, uint32_t addr) const {
  if (addr < 8128) {
...
bool spi_device_reg_top::apply_sw_write(uint32_t addr, uint32_t wdata) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 8128) {
    ...
    switch (index) {
      ...
      case 10: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(4194303) << 2);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 11: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(255) << 8);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 19: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(1) << 25);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 31: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(3) << 22);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 59: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(7) << 16);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          // ignore writes
        }
...
uint32_t spi_device_reg_top::read_sw(uint32_t addr) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 8128) {
    auto &regs = regs_default_;
...
void spi_device_reg_top::set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write) {
