...
static std::string get_progname(char **argv) {
  std::string path(argv && argv[0] ? argv[0] : "");
...
int sc_main(int argc, char **argv) {
  struct top : sc_core::sc_module {
    ...
    opentitan::uart::uart_reg_top dut;
    SC_HAS_PROCESS(top);
  ...
  // Bind RAL soft-access helpers from the DUT model
  tl_bind::ral_sw_read = [&](uint32_t addr){ return top_i.dut.sw_read(addr); };
  tl_bind::ral_sw_predict_write = [&](uint32_t addr, uint32_t data){ top_i.dut.sw_predict_write(addr, data); };
  // Env owns sequencer/driver; no container binding

  apply_uvm_flags(argc, argv);
  if (const char *tn = get_testname_from_argv(argc, argv)) {
    uvm::run_test(tn);
  } else {
    // Default to the program name as test class (matches our binary naming)
    std::string prog = get_progname(argv);
    if (!prog.empty()) uvm::run_test(prog.c_str());
    else uvm::run_test();
  }
  return 0;
}
