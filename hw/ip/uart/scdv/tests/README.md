...
4) Generate coverage HTML (parallel, accumulate across repeats/seeds):
   ...
   util/scdv_cov_report.py --ip uart --outdir hw/ip/uart/scdv/tests/coverage_html      --systemc-home "$SYSTEMC_HOME" --jobs 4 --repeat 1 --extra-args "+UVM_VERBOSITY=UVM_LOW +ntb_random_seed=random" --accumulate --rich
...
Collateral generation:
 - Dry-run scaffold & diff:
   ...
   util/gen_scdv_collateral.py --tmp-out --skip-regtool uart

 - Apply scaffold (writes to repo):

   util/gen_scdv_collateral.py uart
