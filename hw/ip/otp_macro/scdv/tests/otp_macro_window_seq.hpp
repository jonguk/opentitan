...
class otp_macro_window_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(otp_macro_window_seq);
  explicit otp_macro_window_seq(const std::string &nm = "otp_macro_window_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    // Touch a variety of regs to raise addr_cp coverage
    const uint32_t offs[] = {0, 12, 20, 24, 32, 36, 40, 44, 48};
