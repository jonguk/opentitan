...
class otp_macro_csr_rw_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(otp_macro_csr_rw_seq);
  explicit otp_macro_csr_rw_seq(const std::string &nm = "otp_macro_csr_rw_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    const uint32_t offs[] = { 0 };
    const uint32_t wmask[] = { 0 };
    const uint32_t womask[] = { 0 };
    const uint32_t w1cmask[] = { 0 };
    const uint32_t rcmask[] = { 0 };
