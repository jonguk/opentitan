...
2) Build all tests:
   ...
   make -C hw/ip/aon_timer/scdv/tests all

3) Run a specific test (UVM style):

   cd hw/ip/aon_timer/scdv/tests
   ./$aon_timer_csr_rw_test +UVM_TESTNAME=$aon_timer_csr_rw_test +UVM_VERBOSITY=UVM_MEDIUM +ntb_random_seed=random
...
4) Generate coverage HTML (parallel, accumulate across repeats/seeds):
   ...
   util/scdv_cov_report.py --ip aon_timer --outdir hw/ip/aon_timer/scdv/tests/coverage_html      --systemc-home "$SYSTEMC_HOME" --jobs 4 --repeat 1 --extra-args "+UVM_VERBOSITY=UVM_LOW +ntb_random_seed=random" --accumulate --rich
...
Template verification:
 - To verify generated TLM matches checked-in sources, use:
   ...
   util/sc_tlm_diff.py --ip aon_timer  # add --update to sync

Collateral generation:
 - Dry-run scaffold & diff:

   util/gen_scdv_collateral.py --tmp-out --skip-regtool aon_timer

 - Apply scaffold (writes to repo):

   util/gen_scdv_collateral.py aon_timer
