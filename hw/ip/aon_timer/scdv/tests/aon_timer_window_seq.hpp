...
class aon_timer_window_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(aon_timer_window_seq);
  explicit aon_timer_window_seq(const std::string &nm = "aon_timer_window_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    // Touch a variety of regs to raise addr_cp coverage
    const uint32_t offs[] = {44, 12, 20, 24, 32, 36, 40, 44, 48};
