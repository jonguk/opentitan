...
2) Build all tests:
   ...
   make -C hw/ip/adc_ctrl/scdv/tests all

3) Run a specific test (UVM style):

   cd hw/ip/adc_ctrl/scdv/tests
   ./$adc_ctrl_csr_rw_test +UVM_TESTNAME=$adc_ctrl_csr_rw_test +UVM_VERBOSITY=UVM_MEDIUM +ntb_random_seed=random
...
4) Generate coverage HTML (parallel, accumulate across repeats/seeds):
   ...
   util/scdv_cov_report.py --ip adc_ctrl --outdir hw/ip/adc_ctrl/scdv/tests/coverage_html      --systemc-home "$SYSTEMC_HOME" --jobs 4 --repeat 1 --extra-args "+UVM_VERBOSITY=UVM_LOW +ntb_random_seed=random" --accumulate --rich
...
Template verification:
 - To verify generated TLM matches checked-in sources, use:
   ...
   util/sc_tlm_diff.py --ip adc_ctrl  # add --update to sync

Collateral generation:
 - Dry-run scaffold & diff:

   util/gen_scdv_collateral.py --tmp-out --skip-regtool adc_ctrl

 - Apply scaffold (writes to repo):

   util/gen_scdv_collateral.py adc_ctrl
