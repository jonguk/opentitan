...
class adc_ctrl_csr_rw_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(adc_ctrl_csr_rw_seq);
  explicit adc_ctrl_csr_rw_seq(const std::string &nm = "adc_ctrl_csr_rw_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    const uint32_t offs[] = { 0, 4, 8, 12, 0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48 };
    const uint32_t wmask[] = { 0x0, 0xffffffff, 0x0, 0x0, 0x3, 0xfffffff1, 0xff, 0xffff, 0x1, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0x0, 0xffffffff, 0x0, 0x0 };
    const uint32_t womask[] = { 0x0, 0x0, 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
    const uint32_t w1cmask[] = { 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffffffff, 0x0, 0xffffffff, 0x0 };
    const uint32_t rcmask[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
