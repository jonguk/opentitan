...
#include "pattgen_window_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class pattgen_window_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(pattgen_window_test);
  pattgen_env* m_env {};
  explicit pattgen_window_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = pattgen_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = pattgen_window_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(pattgen_window_test);
