...
#include "pattgen_reg_pkg.hpp"
#include <cstring>

namespace opentitan {
namespace pattgen {

pattgen_reg_top::pattgen_reg_top(sc_core::sc_module_name name)
    : sc_core::sc_module(name), tlm_target("tlm_target") {
  tlm_target.register_b_transport(this, &pattgen_reg_top::b_transport);
  reset();
}

void pattgen_reg_top::reset() {
...
void pattgen_reg_top::sw_predict_write(uint32_t addr, uint32_t data) {
  (void)apply_sw_write(addr, data);
}

uint32_t pattgen_reg_top::sw_read(uint32_t addr) {
  return read_sw(addr);
}

void pattgen_reg_top::b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
  (void)delay;
...
bool pattgen_reg_top::check_racl(bool is_write, uint32_t addr) const {
  if (addr < 48) {
...
bool pattgen_reg_top::apply_sw_write(uint32_t addr, uint32_t wdata) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 48) {
    ...
    switch (index) {
      ...
      case 11: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(1023) << 6);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(63) << 16);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(1023) << 22);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
...
uint32_t pattgen_reg_top::read_sw(uint32_t addr) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 48) {
    auto &regs = regs_default_;
...
void pattgen_reg_top::set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write) {
