...
class mbx_csr_rw_seq : public uvm::uvm_sequence<tl_item> {
 public:
  UVM_OBJECT_UTILS(mbx_csr_rw_seq);
  explicit mbx_csr_rw_seq(const std::string &nm = "mbx_csr_rw_seq") : uvm::uvm_sequence<tl_item>(nm) {}
  void body() override {
    ...
    const uint32_t offs[] = { 0, 4, 8, 12 };
    const uint32_t wmask[] = { 0x0, 0xffffffff, 0x0, 0x0 };
    const uint32_t womask[] = { 0x0, 0x0, 0xffffffff, 0x0 };
    const uint32_t w1cmask[] = { 0xffffffff, 0x0, 0x0, 0x0 };
    const uint32_t rcmask[] = { 0x0, 0x0, 0x0, 0x0 };
