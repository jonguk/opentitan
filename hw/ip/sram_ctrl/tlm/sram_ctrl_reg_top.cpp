...
#include "sram_ctrl_reg_pkg.hpp"
#include <cstring>

namespace opentitan {
namespace sram_ctrl {

sram_ctrl_reg_top::sram_ctrl_reg_top(sc_core::sc_module_name name)
    : sc_core::sc_module(name), tlm_target("tlm_target") {
  tlm_target.register_b_transport(this, &sram_ctrl_reg_top::b_transport);
  reset();
}

void sram_ctrl_reg_top::reset() {
  regs_regs_.fill(0u);
  ...
  regs_ram_.fill(0u);
  shadow_stage_ram_.fill(0u);
  shadow_phase_ram_.fill(0u);
  racl_allow_rd_ram_.fill(true);
  racl_allow_wr_ram_.fill(true);
}

void sram_ctrl_reg_top::sw_predict_write(uint32_t addr, uint32_t data) {
  (void)apply_sw_write(addr, data);
}

uint32_t sram_ctrl_reg_top::sw_read(uint32_t addr) {
  return read_sw(addr);
}

void sram_ctrl_reg_top::b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
  (void)delay;
...
bool sram_ctrl_reg_top::check_racl(bool is_write, uint32_t addr) const {
  if (addr < 36) {
  ...
  if (addr < 0) {
    const uint32_t index = addr / kWordBytes;
    return is_write ? racl_allow_wr_ram_[index]
                    : racl_allow_rd_ram_[index];
  }
  return true;
}

bool sram_ctrl_reg_top::apply_sw_write(uint32_t addr, uint32_t wdata) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 36) {
...
uint32_t sram_ctrl_reg_top::read_sw(uint32_t addr) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 36) {
    auto &regs = regs_regs_;
...
void sram_ctrl_reg_top::set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write) {
  if (reg_index < racl_allow_rd_regs_.size()) {
    racl_allow_rd_regs_[reg_index] = allow_read;
    racl_allow_wr_regs_[reg_index] = allow_write;
    return;
  }
  if (reg_index < racl_allow_rd_ram_.size()) {
    racl_allow_rd_ram_[reg_index] = allow_read;
    racl_allow_wr_ram_[reg_index] = allow_write;
    return;
  }
}
