// Copyright lowRISC contributors (OpenTitan project).
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0




#pragma once

#include <systemc>
#include <tlm>
#include <tlm_utils/simple_target_socket.h>
#include <cstdint>
#include <array>

namespace opentitan {
namespace sram_ctrl {

// ----- Types mirrored from sram_ctrl_reg_pkg.sv -----


class sram_ctrl_reg_top : public sc_core::sc_module {
 public:
  tlm_utils::simple_target_socket<sram_ctrl_reg_top> tlm_target;

  SC_HAS_PROCESS(sram_ctrl_reg_top);
  explicit sram_ctrl_reg_top(sc_core::sc_module_name name);

  // Register reset helper
  void reset();

  // Soft-access helpers for DV: bypass TLM socket to read/predict mirror
  void sw_predict_write(uint32_t addr, uint32_t data);
  uint32_t sw_read(uint32_t addr);

  // RACL controls (optional; default disabled and all allowed)
  void set_racl_enable(bool enable) { enable_racl_ = enable; }
  void set_racl_error_response(bool enable) { racl_error_rsp_ = enable; }
  void set_role_mask(uint32_t role_mask) { role_mask_ = role_mask; }
  void set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write);

  // Expose total register bytes for DV convenience
  static constexpr std::size_t kRegBytes = 36;

 private:
  // Backing storage for CSRs (word addressed, regwidth bits)
  static constexpr uint32_t kRegWidthBits = 32;
  static constexpr std::size_t kWordBytes = kRegWidthBits / 8;

  // Memory mapped bus handling
  void b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay);

  // Per-register offsets
  enum class reg_regs_e : uint32_t {
    ALERT_TEST = 0,
    STATUS = 4,
    EXEC_REGWEN = 8,
    EXEC = 12,
    CTRL_REGWEN = 16,
    CTRL = 20,
    SCR_KEY_ROTATED = 24,
    READBACK_REGWEN = 28,
    READBACK = 32,
  };
  enum class reg_ram_e : uint32_t {
  };

  // Aggregate structs mirroring sram_ctrl_reg_pkg

//   ///////////////////////////////////////////////
  // Typedefs for registers for regs interface //
  ///////////////////////////////////////////////

  struct sram_ctrl_reg2hw_alert_test_reg_t {
    sc_dt::sc_bv<1> q;
    bool qe;
  };

  struct sram_ctrl_reg2hw_status_reg_t {
    struct {
      sc_dt::sc_bv<1> q;
    } sram_alert;
    struct {
      sc_dt::sc_bv<1> q;
    } readback_error;
    struct {
      sc_dt::sc_bv<1> q;
    } init_done;
    struct {
      sc_dt::sc_bv<1> q;
    } scr_key_seed_valid;
    struct {
      sc_dt::sc_bv<1> q;
    } escalated;
    struct {
      sc_dt::sc_bv<1> q;
    } init_error;
    struct {
      sc_dt::sc_bv<1> q;
    } bus_integ_error;
  };

  struct sram_ctrl_reg2hw_exec_reg_t {
    sc_dt::sc_bv<4> q;
  };

  struct sram_ctrl_reg2hw_ctrl_reg_t {
    struct {
      sc_dt::sc_bv<1> q;
      bool        qe;
    } init;
    struct {
      sc_dt::sc_bv<1> q;
      bool        qe;
    } renew_scr_key;
  };

  struct sram_ctrl_reg2hw_readback_reg_t {
    sc_dt::sc_bv<4> q;
  };

  struct sram_ctrl_hw2reg_status_reg_t {
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } sram_alert;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } readback_error;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } init_done;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } scr_key_seed_valid;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } scr_key_valid;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } escalated;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } init_error;
    struct {
      sc_dt::sc_bv<1> d;
      bool        de;
    } bus_integ_error;
  };

  struct sram_ctrl_hw2reg_scr_key_rotated_reg_t {
    sc_dt::sc_bv<4> d;
    bool        de;
  };

  // Register -> HW type for regs interface
  struct sram_ctrl_regs_reg2hw_t {
    sram_ctrl_reg2hw_alert_test_reg_t alert_test;
    sram_ctrl_reg2hw_status_reg_t status;
    sram_ctrl_reg2hw_exec_reg_t exec;
    sram_ctrl_reg2hw_ctrl_reg_t ctrl;
    sram_ctrl_reg2hw_readback_reg_t readback;
  };

  // HW -> register type for regs interface
  struct sram_ctrl_regs_hw2reg_t {
    sram_ctrl_hw2reg_status_reg_t status;
    sram_ctrl_hw2reg_scr_key_rotated_reg_t scr_key_rotated;
  };

  // Flat register storage
  std::array<uint32_t, 9> regs_regs_;
  std::array<uint32_t, 0> regs_ram_;

  // Shadow register support (per-register staged/phase; only used when applicable)
  std::array<uint32_t, 9> shadow_stage_regs_{};
  std::array<uint8_t,  9> shadow_phase_regs_{}; // 0:first,1:second
  std::array<uint32_t, 0> shadow_stage_ram_{};
  std::array<uint8_t,  0> shadow_phase_ram_{}; // 0:first,1:second

  // RACL per-register policy (true means allowed)
  std::array<bool, 9> racl_allow_rd_regs_{};
  std::array<bool, 9> racl_allow_wr_regs_{};
  std::array<bool, 0> racl_allow_rd_ram_{};
  std::array<bool, 0> racl_allow_wr_ram_{};

  // Helpers
  bool check_racl(bool is_write, uint32_t addr) const;
  bool apply_sw_write(uint32_t addr, uint32_t wdata);
  uint32_t read_sw(uint32_t addr);

  // Exposed bundles, mirroring SV
  sram_ctrl_regs_reg2hw_t reg2hw_regs;
  sram_ctrl_regs_hw2reg_t  hw2reg_regs;
  sram_ctrl_ram_reg2hw_t reg2hw_ram;
  sram_ctrl_ram_hw2reg_t  hw2reg_ram;

  bool enable_racl_ = false;
  bool racl_error_rsp_ = true;
  uint32_t role_mask_ = 0; // externally assigned; model treats nonzero as privileged
};

}  // namespace sram_ctrl
}  // namespace opentitan


