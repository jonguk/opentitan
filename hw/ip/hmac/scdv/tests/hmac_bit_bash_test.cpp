...
#include "hmac_bit_bash_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class hmac_bit_bash_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(hmac_bit_bash_test);
  hmac_env* m_env {};
  explicit hmac_bit_bash_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = hmac_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = hmac_bit_bash_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(hmac_bit_bash_test);
