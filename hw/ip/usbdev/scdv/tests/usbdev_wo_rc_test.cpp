...
#include "usbdev_wo_rc_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class usbdev_wo_rc_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(usbdev_wo_rc_test);
  usbdev_env* m_env {};
  explicit usbdev_wo_rc_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = usbdev_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = usbdev_wo_rc_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(usbdev_wo_rc_test);
