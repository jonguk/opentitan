...
#include "otbn_shadow_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class otbn_shadow_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(otbn_shadow_test);
  otbn_env* m_env {};
  explicit otbn_shadow_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = otbn_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = otbn_shadow_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(otbn_shadow_test);
