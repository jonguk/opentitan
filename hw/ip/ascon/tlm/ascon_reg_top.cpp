...
#include "ascon_reg_pkg.hpp"
#include <cstring>

namespace opentitan {
namespace ascon {

ascon_reg_top::ascon_reg_top(sc_core::sc_module_name name)
    : sc_core::sc_module(name), tlm_target("tlm_target") {
  tlm_target.register_b_transport(this, &ascon_reg_top::b_transport);
  reset();
}

void ascon_reg_top::reset() {
...
void ascon_reg_top::sw_predict_write(uint32_t addr, uint32_t data) {
  (void)apply_sw_write(addr, data);
}

uint32_t ascon_reg_top::sw_read(uint32_t addr) {
  return read_sw(addr);
}

void ascon_reg_top::b_transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
  (void)delay;
...
bool ascon_reg_top::check_racl(bool is_write, uint32_t addr) const {
  if (addr < 188) {
...
bool ascon_reg_top::apply_sw_write(uint32_t addr, uint32_t wdata) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 188) {
    ...
    switch (index) {
      ...
      case 37: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(3) << 3);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 40: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(4095) << 12);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(31) << 24);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          newv = (newv & ~mask) | raw;
        }
      ...
      case 43: {
        ...
        {
          constexpr uint32_t mask = (static_cast<uint32_t>(3) << 2);
          const uint32_t raw = wdata & mask;
          const uint32_t cur = newv & mask;
          // ignore writes
        }
...
uint32_t ascon_reg_top::read_sw(uint32_t addr) {
  const uint32_t index = addr / kWordBytes;
  if (addr < 188) {
    auto &regs = regs_default_;
...
void ascon_reg_top::set_reg_policy(std::size_t reg_index, bool allow_read, bool allow_write) {
