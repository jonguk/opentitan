...
#include "kmac_racl_seq.hpp"
#include "../../../dv/sc/tl_agent/tl_sequencer.hpp"
#include "../../../dv/sc/tl_agent/seq_lib/tl_seq_list.hpp"
using namespace uvm;

class kmac_racl_test : public uvm_test {
 public:
  UVM_COMPONENT_UTILS(kmac_racl_test);
  kmac_env* m_env {};
  explicit kmac_racl_test(uvm_component_name nm) : uvm_test(nm) {}
  void build_phase(uvm_phase &phase) override { m_env = kmac_env::type_id::create("env", this); }
  void run_phase(uvm_phase &phase) override {
    phase.raise_objection(this);
    auto seq = kmac_racl_seq::type_id::create("seq");
...
UVM_COMPONENT_REGISTER(kmac_racl_test);
